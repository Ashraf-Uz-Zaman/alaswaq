...
class SearchFromHome extends StatefulWidget {
  String name;
  SearchFromHome({this.name});

  @override
  SearchFromHomeState createState() => SearchFromHomeState();
}

class SearchFromHomeState extends State<SearchFromHome> {
                  ...
                  Row(
                    children: [
                      GestureDetector(
                        onTap: () async {
                          _key.currentState
                              .showBottomSheet((context) => CatButtonFilters("From Search Area", widget.name, catName));
                        },
                        child: Image.asset(
...
//                                          producrtPrice(index, productList.items[index].sku);
                                          Navigator.push(
                                            context,
                                            MaterialPageRoute(builder: (context) => ConfirabaleProducts(productList, index,
...
//                                                fontWeight: FontWeight.bold,
  ...
  Future getProductList(String key) async {
    ApiRepository.loadHomeSearch(key).then((userDataFromServer) {
...
Future addCartGuest(String skuVAl, int qty) async {
  _key.currentState.showSnackBar(
    ...
    print("guest cart added search home");
    print(response);
    final responseJson = json.decode(response.body);
    print(responseJson);
...
//
  ...
  Future getSortbyPosition() async {
    ...
    ApiRepository.sortOnPostionForName(widget.name).then((userDataFromServer) async {
      if (userDataFromServer != null) {
        setState(() {
          _progressController = false;
          productList = userDataFromServer;
  ...
  Future getSortbyName() async {
    ...
    ApiRepository.sortOnNameForName(widget.name).then((userDataFromServer) async {
      if (userDataFromServer != null) {
        setState(() {
          _progressController = false;
          productList = userDataFromServer;
  ...
  Future getSortbyPrice() async {
    ...
    ApiRepository.sortOnPriceForName(widget.name).then((userDataFromServer) async {
      if (userDataFromServer != null) {
        setState(() {
          _progressController = false;
          productList = userDataFromServer;
